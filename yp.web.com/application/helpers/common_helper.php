    ...
    foreach ($rulesData as $rule) {
        ...
        switch ($filter) {
            ...
            case 'email':
                ...
                break;
            case 'phone':
                if (!preg_match('/^1[34578]\d{9}$/', $data, $match)) {
                    $res['field'] = '';
                    $res['code'] = 'validation_error_6'; // 手机号码不正确
                }
                break;
            default:
...
function baseCssUrl($uri){
    ...
    return $this->getBaseUrl().'/assets/css/' . $uri;
}

function baseJsUrl($uri){
    return $this->getBaseUrl().'/assets/js/' . $uri;
}

// 获取common配置信息
function getCommonConf($key){
    $ci = &get_instance();
