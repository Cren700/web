...
class Account_service_model extends MY_Model
{
    ...
    public function addAccount($username, $pwd)
    {
        ...
        $validationConfig = array(
            array(
                'value' => $username,
                'rules' => 'required|phone|min_length[4]|max_length[16]',
                'field' => '用户名'
            ),
            array(
                'value' => $pwd,
        ...
        if ($this->account_dao_model->addAccount($username, $pwdCode, $salt) ){
            return array('code' => 0, 'url' => $this->config->item('base_url'));
        } else {
    ...
    public function accountLogin($username, $pwd)
    {
        ...
        if ($info['pwd'] !== $pwdCode) {
            ...
            $resData = array('id' => $info['id'], 'username' => $info['username'], 'url' => '/merchant/index.html');
