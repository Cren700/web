...
class Captcha
{
    ...
    function __construct($width=100, $height=40, $codeNum=4){
        $this->width = $width;
        $this->height = $height;
...
//        $str = "23456789abcdefghijkmnpqrstuvwxyzABCDEFGHIJKMNPQRSTUVWXYZ";
    ...
    private function setCaptcha(){
        for ($i = 0; $i < $this->codeNum; $i++) {
            $color = imagecolorallocate($this->im, rand(0, 125), rand(0, 150), rand(0, 150));
//            $size = rand(floor($this->height / 5), floor($this->height / 3));
            $size = 5;
            $x = floor($this->width / $this->codeNum) * $i + 5;
            $y = rand(0, floor($this->height / 2 ));
